...
const PostCard = ({ post, onDelete, onUpdate }) => {
  ...
  return (
    <Card data-testid={`post-card-${post.id}`} className="hover:shadow-md transition-shadow duration-200">
          ...
          <div className="flex-1">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p data-testid="post-timestamp" className="text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-6 mt-4">
              <Button
                ...
                size="sm"
                onClick={handleLike}
                className={`gap-2 rounded-full ${isLiked ? 'text-red-500' : ''}`}
              >
                <Heart className={`w-5 h-5 ${isLiked ? 'fill-current heart-pop' : ''}`} />
                <span data-testid="post-likes-count">{likesCount}</span>
              </Button>
              ...
              >
                ...
                <span data-testid="post-comments-count">{post.comments_count}</span>
              </Button>
